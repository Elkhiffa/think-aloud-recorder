...
function renderControls(){renderHome();const locked=!store.connected||store.requestPending||store.busy;$('#recordButton').disabled=store.recording?(!store.connected||store.requestPending||!!store.activity.busy):!store.canStart;$('#presetSelect').disabled=locked||!store.presets.length;$('#newPresetButton').disabled=locked;$('#settingsButton').disabled=locked||!store.activeId;$('#openVault').disabled=!store.connected||store.requestPending||!store.activeId;
 ...
 $$('[data-provider]').forEach(button=>button.disabled=fieldsLocked);
 ...
 const m=store.snapshot?.model||{};$('#downloadModel').disabled=locked||saving||['verifying','ready'].includes(m.state);$('#existingModel').disabled=locked||saving||['downloading','verifying'].includes(m.state);$('#modelLocation').disabled=locked||saving||['downloading','verifying'].includes(m.state);$('#verifyCloud').disabled=locked||saving||!store.snapshot?.capabilities?.cloud_key||!!$('#cloudKey').value.trim();$$('[data-session-action]').forEach(b=>b.disabled=(!store.connected||store.requestPending||store.snapshot?.closing||(!['review','folder'].includes(b.dataset.sessionAction)&&store.busy)||sessionActionBlocked(b.dataset.id,b.dataset.sessionAction)));}
...
function fieldChanged(element){if(element.id==='source'||element.id==='mic')pendingDeviceSetup=null;store.updateDraft(element.dataset.draft,element.value);if(element.id==='source'){deviceSignature='';renderDeviceOptions(true);}if(element.dataset.draft==='hotword_manual')renderVocabulary();renderSummary();}
...
function sessionRow(s){const kind=sessionKind(s),job=store.sessionJob(s.id),state=job?(job.state==='queued'?'等待整理':'后台整理'):s.state||'状态未知';const progress=job?(job.state==='queued'?'已保存，等待前面的场次整理完成。':job.detail||'正在整理，仍可继续录制。'):'';return `<article class="session-row"><div class="session-main"><div class="session-name"><strong>${escapeHTML(s.game||s.id)}${s.test?' <span class="test-tag">合成测试</span>':''}</strong><p class="session-meta"><span>${escapeHTML(formatDate(s.created))}</span><span class="session-duration"><svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="12" cy="12" r="8"/><path d="M12 7v5l3 2"/></svg>${escapeHTML(formatTime(s.duration))}</span></p>${progress?`<p class="session-progress">${escapeHTML(progress)}</p>`:''}</div><span class="session-state ${kind}">${escapeHTML(state)}</span>${kind==='ready'?`<button class="session-quick" data-session-action="review" data-id="${escapeHTML(s.id)}">${icon('play')}回看</button>`:['pending','failed'].includes(kind)?`<button class="session-quick" data-session-action="process" data-id="${escapeHTML(s.id)}">${kind==='failed'?'重试':'整理'}</button>`:''}<button class="session-toggle" data-detail="${escapeHTML(s.id)}" aria-expanded="${expandedSession===s.id}" aria-label="${expandedSession===s.id?'收起':'展开'}${escapeHTML(s.game||'场次')}详情">${icon('chevron')}</button></div>${expandedSession===s.id?sessionDetail(s):''}</article>`;}
