...
const out=path.join(root,'work','input-review-bands');fs.mkdirSync(out,{recursive:true});
...
async function main(){
 ...
 await check('hot-switch overlap retains both devices and a controller gap does not erase keyboard evidence',async()=>{
   ...
   await page.waitForFunction(()=>document.querySelectorAll('#currentKeys .keycap').length===4);assert.equal(await page.locator('.key-bar.active').count(),3);assert.equal(await page.locator('#inputSource').textContent(),'Xbox');assert.equal(await page.locator('#currentKeys').isVisible(),true);
 ...
 await check('unchanged revision skips 100k history serialization and DOM reset; changed revision refreshes without media reset',async()=>{
   ...
   await page.waitForFunction(()=>!document.querySelector('#transcriptTab').disabled&&document.querySelector('#timelineState').textContent.startsWith('100018'));
 ...
 const denseInputs={version:1,state:'complete',duration:32,intervals:[{id:'dense-w',device:'keyboard',code:'E',label:'E',kind:'button',start:.5,end:7},...denseAxis,...denseMouse,...denseTaps],gaps:[]};
 ...
 await check('continuous analog samples form activity bands while current input stays raw',async()=>{
   ...
   assert.equal(await page.locator('.bar-direction').count(),0);assert.ok(await page.locator('[data-input-id="dense-axis-0"] .input-mark').count()<=1);assert.ok((await page.locator('[data-input-id="dense-axis-0"] .bar-glyph').textContent()).includes('↑'));
 ...
 await check('short presses use readable display boxes and leftmost nonoverlapping lanes at every zoom',async()=>{
   for(const delta of [240,240,-240,-240,-240,-240]){
     ...
     for(const tap of taps){const occupied=rows.filter(other=>other.channel==='other'&&other.start<tap.start&&other.rect.bottom+3.9>tap.rect.y).map(other=>other.lane);assert.equal(tap.lane,Array.from({length:21},(_,i)=>i).find(i=>!occupied.includes(i)));}
   }
   await seek(2.15);const tap=page.locator('[data-input-id="dense-tap-2"]');assert.equal(await tap.locator('.dpad-glyph').count(),1);assert.ok(!(await tap.locator('.bar-glyph').textContent()).includes('DPadDown'));
 ...
 await check('horizontal track browsing keeps headers aligned and video fixed without hiding events',async()=>{
   ...
   const rows=await visualRows(),firstOther=rows.filter(item=>item.channel==='other'&&item.lane===0)[0];assert.ok(Math.abs(firstOther.rect.x-alignment.header[5].x-4)<1);assert.equal(rows.filter(item=>item.id.startsWith('dense-tap-')).length,17);
 ...
 await check('key drag scrubs against frozen coordinates through a snapshot rebuild and never becomes a click',async()=>{
   ...
   const helpers=require('../ui/review.js');const expected=helpers.recentInputs(helpers.intervalIndex(helpers.normalizeInputs(denseInputs).intervals),current.time).length;assert.equal(await page.locator('#currentKeys .current-item').count(),expected);
 ...
 await check('recent operations expire independently and timeline layers retain the intended order',async()=>{
   await page.setViewportSize({width:1280,height:800});
   const recent={state:'complete',duration:32,intervals:[
     {id:'recent-a',device:'xbox',code:'A',kind:'button',start:1,end:1.1},
     {id:'recent-b',device:'xbox',code:'B',kind:'button',start:1.8,end:1.9},
     {id:'recent-hold',device:'xbox',code:'RB',kind:'button',start:2.5,end:6},
     {id:'dpad-1',device:'xbox',code:'DPadDown',kind:'button',start:2,end:2.1},
     {id:'dpad-2',device:'xbox',code:'DPadRight',kind:'button',start:2.6,end:2.7}
   ],gaps:[]};
   await page.evaluate(inputs=>{window.__snapshot.inputs=inputs;window.__snapshot.revision='recent-check';},recent);
   await page.waitForFunction(()=>document.querySelector('[data-input-id="recent-a"]'));
   await seek(2.8);assert.equal(await page.locator('#currentKeys .current-item').count(),5);assert.equal(await page.locator('#currentKeys .active').count(),1);
   await seek(3.15);assert.equal(await page.locator('[data-recent-id="recent-a"]').count(),0);assert.equal(await page.locator('[data-recent-id="recent-b"]').count(),1);
   await seek(3.91);assert.equal(await page.locator('[data-recent-id="recent-b"]').count(),0);
   await seek(6.1);assert.equal(await page.locator('[data-recent-id="recent-hold"] .recent').count(),1);
   await seek(8);assert.equal(await page.locator('#currentKeys .current-item').count(),0);
   await seek(2.4);assert.equal(await page.locator('[data-recent-id="recent-hold"]').count(),0);
   assert.ok((await page.locator('[data-input-id="dpad-1"] .bar-glyph').textContent()).includes('↓'));
   assert.equal(await page.locator('[data-input-id="dpad-1"] .bar-glyph').isVisible(),true);
   await seek(2.65);assert.ok((await page.locator('[data-input-id="dpad-1"] .bar-glyph').textContent()).includes('→'));
   await seek(2.8);assert.equal(await page.locator('[data-input-id="dpad-1"] .bar-glyph').isVisible(),false);
   const layers=await page.evaluate(()=>Object.fromEntries(['.timeline-ticks','.timeline-ruler','.timeline-inputs','.timeline-playhead'].map(s=>[s,+getComputedStyle(document.querySelector(s)).zIndex])));
   assert.ok(layers['.timeline-ticks']<layers['.timeline-inputs']);assert.ok(layers['.timeline-inputs']<layers['.timeline-playhead']);
   assert.equal(await page.locator('.timeline-ticks time').count(),0);assert.ok(await page.locator('.timeline-ruler time').count()>0);
   await settleVideo();await page.screenshot({path:path.join(out,'recent-inputs-layering.png')});
 });
 await check('per-clip time adjustment previews immediately, persists, cancels and resets without moving the video',async()=>{
   await seek(2.8);const before=await playback();
   await page.locator('#fileActionsToggle').click();await page.locator('#alignInputs').click();
   await page.locator('#inputOffset').fill('1.8');
   assert.equal(await page.locator('[data-recent-id="recent-b"]').count(),0);
   assert.equal(await page.locator('[data-input-id="recent-a"]').getAttribute('data-start'),'2.8');
   assert.equal((await playback()).time,before.time);
   await page.locator('#cancelAlignment').click();assert.equal(await page.locator('[data-input-id="recent-a"]').getAttribute('data-start'),'1');
   await page.locator('#fileActionsToggle').click();await page.locator('#alignInputs').click();await page.locator('#inputOffset').fill('1.8');await page.locator('#alignmentForm [type=submit]').click();
