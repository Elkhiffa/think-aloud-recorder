...
### Hold names, uniform key columns and quote placement

Long-hold names are painted above their fills, including stable LT/L2 trigger samples inside a changing analog band. Names start at the top of a hold and track the visible top when it scrolls away, bounded by that hold's end. Fixed movement tracks retain their playhead direction indicator alongside hold headers. Ordinary key lanes share one width, sized to the longest key name in the recording; short taps and holds align to the same grid through zoom and horizontal browsing. Time, speech and movement remain separate dedicated tracks.

Quote previews open to the left of the speech marker, with their right edge 12 pixels before the marker. Viewport clamping and manually dragged pinned positions continue to apply. Regression uses production browser assets plus synthetic media. The native WebView2 smoke in `work/native-hold-labels/acceptance.json` verified a scrolled LT header at the viewport top, unchanged playback time, left-positioned quote, and clean window close; no OBS or physical capture was started.

Device diagrams also reconstruct recent input from video time: held controls use full theme red; released controls begin at 70% intensity and fade linearly to their normal appearance over two media seconds. Each control expires independently, repeated presses restore full intensity, pausing freezes the evidence and seeking recomputes it. Keyboard, mouse, controller controls, stick arrows, trigger meters and extra keys use the same timing. Trigger width still uses recorded analog values; color encodes recency, not inferred physical pressure. The recent-keys list retains its existing separate-event representation.

## 2026-09-24: foreground readiness and saved game-window identity

Automatic device/readiness checks now require the native main recorder window to be foreground on the unlocked input desktop. A review window, browser visibility state, or UI polling cannot admit a check. Before the main window exists, automatic OBS initialization waits. Returning to the main window requests a fresh check immediately; recording health checks remain independent. An already-issued OBS request may finish and release its temporary resources after focus changes. This is not a guarantee that OBS itself can never crash on workstation lock.

The reported lock-time crash occurred in OBS's main thread at `signal_handler_disconnect`, reached through `pthread_mutex_unlock`, during the same second as repeated temporary source creation/removal. That is evidence of a source-lifecycle correlation, not proof of an exact upstream fault or a GPU-driver diagnosis. No OBS binary or driver was replaced. The native focus smoke uses real WebView2 windows and foreground changes; SessionLock/SessionUnlock notifications are injected. Actual workstation lock was not performed.

Saved OBS window strings contain title, window class and executable. Selection first requires a unique exact match, then a unique same-title/executable match, then a unique executable match. Disabled entries, malformed fallback identities and ambiguous matches do not select another target. Start re-enumerates and verifies the resolved identity before enabling the window source. Session metadata and strict HWND/process-bound input capture consume that same resolved identity; saved presets are not silently rewritten. A running game inspection found the current saved and live class identical, so the user's historic failures cannot be attributed conclusively to a particular game update.

Browser evidence in `work/window-selection-browser-v2/acceptance.json` covers the live-match label, manual selection across polling, cancel without persistence, and disabled Start for ambiguous matches. Native foreground evidence is retained in `work/native-focus-v2/acceptance.json`. Both use synthetic bridge/device fixtures and do not establish actual game recording or physical input accuracy.

### Startup preparation is separate from interruption and alignment

Startup status follows actual readiness, OBS start acknowledgement and input-listener setup. An input-listener failure leaves the video running with an explicit operation-recording warning, including after the next health check. No fixed countdown delays recording or substitutes for a successful acknowledgement.

For complete input sidecars without an error, the exact leading `录像启动确认前尚未采集操作` gap is presented as neutral `录制准备` and excluded from the abnormal-gap count. Other capture gaps, device disconnections and target-focus gaps keep their original classification. No video, input sidecar or speech timestamp is rewritten.

The first media load may seek, without playing, to this preparation gap's end after the existing measured or manual input offset has been applied once. This requires a trustworthy alignment and a preparation end inside the video. Failed or uncalibrated recordings remain at zero. Earlier user interaction or an explicit transcript selection wins; transcript refreshes and later offset edits never cause another automatic seek. The full original beginning remains seekable.

Evidence: `tests/test_review_ui.js` checks positive/negative offsets, manual alignment, real focus gaps, failed and malformed preparation, clipping and raw-data immutability. `work/startup-review/acceptance.json` uses production browser assets with synthetic video/input data and verifies seven opening, browsing and refresh scenarios. No new actual OBS recording or workstation lock was performed for this presentation change.
